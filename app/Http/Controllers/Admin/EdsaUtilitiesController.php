...
use App\Models\EdsaSubscription;
use App\Models\User;
class EdsaUtilitiesController extends AdminController
{
    //
    public function index()
    {
        // $edsaUtilities = EdsaUtilities::first();
        return view('admin.edsautilities.view');
    }
    ...
    public function saveMeter(Request $request)
    {
        ...
        if ($user) {
            ...
            return redirect()->back()->with('success', 'Meter saved successfully.');
        } else {
            // If the user doesn't exist, return with an error message
            return redirect()->back()->with('error', 'User not found.');
        }
    }
    public function edsa_subscription(){
        $users_seller = [];
        $dstv = EdsaSubscription::all();
        ...
        // return $users_seller;
        return view('admin.edsautilities.edsa_subscription',compact('dstv','users_seller'));
    }
    public function edsa_subscription_store(Request $request){
        // return $request;
        $subscription = new EdsaSubscription();
    ...
    public function edsa_subscription_edit($id){
        // return $id;
        $users_seller = [];
        $users = User::where('is_seller', 0)->where('is_edsa_agent', 1)->get();
        ...
        $edsa_subscription = EdsaSubscription::where('id',$id)->first();
        return view('admin.edsautilities.edit_edsa_subscription',compact('edsa_subscription','users_seller'));
    }
    public function edsa_subscription_update(Request $request){
        $subscription = EdsaSubscription::where('id',$request->subscription_id)->first();
        $subscription->user_id = $request->users;
        $subscription->role = $request->role;
        $subscription->go_plan_monthly = $request->go_plan_monthly;
        $subscription->go_plan_referal_fee = $request->go_plan_referal_fee;
    ...
    public function edsa_subscription_delete($id){
        // return $id;
        $subscription = EdsaSubscription::where('id',$id)->first();
        $subscription->delete();
