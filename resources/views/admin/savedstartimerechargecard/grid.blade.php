...
@section('grid-content')
    ...
    <div class="card">
        <div class="header">
            <h2>
                Saved Star Times Recharge Card Listing
            </h2>
        </div>

            <!-- @php
                echo "test";
                foreach($saveddstvrechargecard as $d) {
                    echo $d;
                }
            @endphp -->
            <div class="body">
                <div class="row">
                    ...
                    @if ($saveddstvrechargecard->count())
                        <table class="table">
                            <tbody>
                            <th>Recharge Card Number</th>
                            <th>Username</th>
                            <th>Rechange Card Name</th>
                            <th>Email</th>
                            <th>Mobile Number</th>
                            <th>Created At</th>
                            </tbody>
                            <tbody>
                            @foreach($saveddstvrechargecard as $user)
                                <tr>
                                    <td>{{ $user->recharge_card_number }}</td>
                                    <td><a href="{{ route('admin.user.show', $user->user_id) }}" >{{ $user->user->username }}</a></td>
                                    <td>{{ $user->recharge_card_name }}</td>
